#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Time-stamp: "2022-11-17 17:52:52 (ywatanabe)"
...
for sd in [2.0]:
    ...
    rips_df["Ripple duration [ms]"] = (
        rips_df["end_time"] - rips_df["start_time"]
    ) * 1000
    rips_df["log(ripple duration [ms])"] = np.log10(
        (rips_df["end_time"] - rips_df["start_time"]) * 1000
    )

    rips_df["log(ripple_peak_amplitude_sd)"] = np.log10(
    ...
    for phase in ["Fixation", "Encoding", "Maintenance", "Retrieval"]:
        ...
        for i_ax, ax in enumerate(axes.ravel()):
            ...
            is_correct, match_str = list(
                product([True, False], ["match IN", "mismatch OUT"])
            )[i_ax]
        ...
        mngs.io.save(
            fig,
            f"./tmp/figs/duration_and_set_size/{iEEG_POSITIONS_STR}/correct_only/{phase}_{sd}_SD.png",
        )
