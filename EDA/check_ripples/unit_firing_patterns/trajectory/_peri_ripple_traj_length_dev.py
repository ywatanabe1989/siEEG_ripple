    ...
    for event in ["SWR", "Control"]:
        for iep, event_phase in enumerate(PHASES):
            for ibp, (base_phase_1, base_phase_2) in enumerate(
                ...
                # print(
                #     event, event_phase, base_phase_1, base_phase_2, round(p_val_corr, 3)
                # )
...
def print_unc_pvals_corr(df_length_stats):
    for phase in PHASES:
        tmp = df_length_stats.T[(df_length_stats.T["Event phase"] == phase) \
            * df_length_stats.T["Base direction"] == f"g{phase[0]}_4 to g{phase[0]}_8 direction"
                   ]
        print(phase)
...
def get_correlations(df_length):
    ...
    for phase in PHASES:
        ...
        # only the direction from g4 to g8
        # df_length_length_phase_directed = df_length[
        #     (df_length.event_phase == phase)
        # ]
        # # only the direction from g4 to g8
        
        df_length_phase_directed = df_length_phase_directed[["length", "Event", "set_size"]]
        df_length_phase_directed = df_length_phase_directed.sort_values(["Event", "set_size"])
