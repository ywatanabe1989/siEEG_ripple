#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Time-stamp: "2023-04-06 13:28:05 (ywatanabe)"
...
if __name__ == "__main__":
    ...
    for subject, roi in ROIs.items():
        ...
        for session in ["01", "02"]:
                ...
                # traj = mngs.io.load(
                #     f"./data/Sub_{subject}/Session_{session}/traj_z_by_session_{roi}_match_None_wo_R.npy"
                # )
                ...
                dists_z_in.append(dist[trials_info.match == 1])
                dists_z_out.append(dist[trials_info.match == 2])

    dists_z_in = np.vstack(dists_z_in)
    dists_z_out = np.vstack(dists_z_out)                    


    mm_in = np.nanmean(dists_z_in, axis=0)
    sd_in = np.nanstd(dists_z_in, axis=0)
    nn_in = np.sum(~np.isnan(dists_z_in), axis=0)
    ci_in = 1.96 * sd_in / nn_in

    mm_out = np.nanmean(dists_z_out, axis=0)
    sd_out = np.nanstd(dists_z_out, axis=0)
    nn_out = np.sum(~np.isnan(dists_z_out), axis=0)
    ci_out = 1.96 * sd_out / nn_out
    
    fig, ax = plt.subplots()
    # xx = np.linspace(0,6,120)
    xx = np.linspace(0,8,160)        
    for mm, sd, label in zip([mm_in, mm_out], [sd_in, sd_out], ["IN", "OUT"]):
        ax = mngs.plt.ax_fill_between(ax, xx, mm, sd, label)
    # for mm, ci, label in zip([mm_in, mm_out], [ci_in, ci_out], ["IN", "OUT"]):
    #     ax = mngs.plt.ax_fill_between(ax, xx, mm, ci, label)
        # ax.plot(xx, mngs.dsp.gaussian_filter1d(mm, 10))
        # ax.plot(xx, np.zeros(120))
        ax.axvline(x=1)
        ax.axvline(x=3)    
        ax.axvline(x=6)
