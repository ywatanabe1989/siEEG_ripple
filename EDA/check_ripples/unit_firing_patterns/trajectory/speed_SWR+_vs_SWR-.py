#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Time-stamp: "2023-02-20 10:17:19 (ywatanabe)"
"""
    ...
    for subject, roi in ROIs.items():
        ...
        for session in ["01", "02"]:
            ...
            rips_mask = utils.rips.mk_rips_mask(
                rips_df, subject, session, roi, 250
            )  # (49, 160)
            masked_speed_rips = speed_session * rips_mask[..., :-1]                        
...
def plot(xx, speeds_rips, speeds_cons, is_sign, effs):
    fig, ax = plt.subplots()
    ax.plot(xx, np.array(effs)*10, label="effs")
    ax = mngs.plt.ax_fill_between(
        ax, xx, np.nanmean(speeds_rips, axis=0), np.nanstd(speeds_rips, axis=0), "SWR+"
    )
    ax = mngs.plt.ax_fill_between(
        ax, xx, np.nanmean(speeds_cons, axis=0), np.nanstd(speeds_cons, axis=0), "SWR-"        
    )
    ax.scatter(xx[is_sign], 4*np.ones(is_sign.sum()))
    ax.legend()
    [ax.axvline(x=_x, linestyle="--", color="gray") for _x in [-5, -3, 0, 2]]
    ax.set_ylabel("Peri-SWR speedance from O [a.u.]")
...
if __name__ == "__main__":
    ...
    # speeds_rips[speeds_rips == 0] = np.nan
    # speeds_cons[speeds_cons == 0] = np.nan    
