#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Time-stamp: "2023-03-13 16:00:55 (ywatanabe)"
...
def calc_SWR_speed_in_direction_of_Encoding_SWR(subject, session, match, phase):
    rips_df_m_s = rips_df[
    ...
    rebased = mngs.gen.listed_dict()
    for phase in PHASES:
        tgt_vecs = rips_df_m_s[rips_df_m_s.phase == phase]["speed_vec"]
        base_vecs = rips_df_m_s[rips_df_m_s.phase == "Encoding"]["speed_vec"]
        for tgt_vec in tgt_vecs:
            for base_vec in base_vecs:
                rebased[phase].append(mngs.linalg.rebase_a_vec(tgt_vec, base_vec))
    return mngs.gen.force_dataframe(rebased, filler=np.nan)
...
if __name__ == "__main__":
    ...
    rips_df["speed_vec"] = rips_df["2"] - rips_df["-2"]
    match = 1

    fig, axes = plt.subplots(ncols=2, sharex=True, sharey=True)    
    for i_match, match in enumerate([1,2]):
        dfs = []
        for subject in rips_df.subject.unique():
            for session in rips_df.session.unique():            
                # subject = "06"
                # session = "02"
                df = calc_SWR_speed_in_direction_of_Encoding_SWR(subject, session, match, phase)
                dfs.append(df)
        df = pd.concat(dfs)
        mngs.io.save(df, f"./tmp/figs/box/SWR_direction_in_the_eSWR_direction/match_{match}/all.csv")

        ax = axes[i_match]

        for i_col, col in enumerate(df.columns):
            data = df[col][~df[col].isna()]
            ax.boxplot(data, positions=[i_col], showfliers=False)
        # df.apply(mngs.gen.describe)
    plt.show()
