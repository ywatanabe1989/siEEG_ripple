#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Time-stamp: "2023-01-16 00:37:13 (ywatanabe)"
...
def load_rips_df_with_traj(bin_size, is_control=False):
    ...
    if is_control:
        ...
        cons_df["center_time"] = ((cons_df["end_time"] + cons_df["start_time"]) / 2).astype(float)
        rips_df = cons_df
...
def plot_norm(rips_df, is_control=False, set_size=None):
    ...
    n_bases = len([None, "Fixation", "Fixation", "Fixation", "Encoding", "Encoding", "Maintenance"])
    ...
    for i_base, (cbs, cbe) in enumerate(
        zip(
            ...
            [None, "Fixation", "Fixation", "Fixation", "Encoding", "Encoding", "Maintenance"],
            [None, "Encoding", "Maintenance", "Retrieval", "Maintenance", "Retrieval", "Retrieval"],
        )
    ):
        ...
        for i_phase, phase in enumerate(PHASES):
            ...
            for delta_bin in range(-8, 9):
                ...
                # print(n_samp)
                sd = np.nanstd(v_rebased) / 3
                samp_s[phase].append(sd)
                # if n_samp != 0:
    ...
    # plt.show()
    mngs.io.save(fig, f"./tmp/figs/hist/traj_norm/all_{rips_or_cons_str}{set_size_str}.png")
    mngs.io.save(out_df, f"./tmp/figs/hist/traj_norm/all_{rips_or_cons_str}{set_size_str}.csv")
    return fig


def plot_positions(rips_df, is_control=False, set_size=None):
    rips_or_cons_str = "cons" if is_control else "rips"
    ...
    mngs.io.save(fig, f"./tmp/figs/hist/traj_pos/all_{rips_or_cons_str}{set_size_str}.png")
    mngs.io.save(out_df, f"./tmp/figs/hist/traj_pos/all_{rips_or_cons_str}{set_size_str}.csv")
    return fig
    # plt.show()
...
if __name__ == "__main__":
    ...
    for is_control in [False, True]:
        rips_df = load_rips_df_with_traj(bin_size, is_control=is_control)
        ...
        for set_size in [None, 4, 6, 8]:
            fig = plot_norm(rips_df, is_control=is_control, set_size=set_size) # fig 4
            fig = plot_positions(rips_df, is_control=is_control, set_size=set_size) # fig 5
            plt.close()
            plt.close()            
    plt.show()
