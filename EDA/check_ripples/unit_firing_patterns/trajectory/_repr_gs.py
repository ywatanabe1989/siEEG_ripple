#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Time-stamp: "2023-02-17 09:10:16 (ywatanabe)"
...
for phase, (bin_start, bin_end) in GS_BINS_DICT.items():
    ...
    gs[f"{phase[0]}_z"] = g[2]            
    print(phase)
mngs.io.save(pd.DataFrame(med_traj).T, "./tmp/figs/scatter/repr_med_traj.csv")
mngs.io.save(pd.DataFrame(pd.Series(gs)).T, "./tmp/figs/scatter/repr_med_traj_gs.csv")
