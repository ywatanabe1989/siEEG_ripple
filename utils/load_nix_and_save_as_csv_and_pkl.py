#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Time-stamp: "2022-11-20 12:53:20 (ywatanabe)"
...
def load_signals(f, iEEG_ROI=["PHL", "PHR"]):
    def _in_load_signals_trial(f, trial_number=1):
        ...
        # spike times
        st_keys = mngs.general.search("^Spike_Times_",
        ...
        for st_key in st_keys:
            st_dict[st_key] = np.array(f["data"][list(f["data"].keys())[0]]["data_arrays"][        
                st_key
            ]["data"])
...
if __name__ == "__main__":
    ...
    for iEEG_ROI in ["AHL", "AHR", "PHL", "PHR", "ECL", "ECR", "AL", "AR"]:
        ...
        for fpath in fpaths:
            ...
            # i_Sub
            i_sub_str = re.findall("Subject_[\0-9]{2}", fpath)[0][-2:]

            # i_Session
            i_session_str = re.findall("Session_[\0-9]{2}", fpath)[0][-2:]
            ...
            iEEG, EEG, spike_times = _signals["iEEG"], _signals["EEG"], _signals["spike_times"]
            
            # saves
            session_dir = f"./data/Sub_{i_sub_str}/Session_{i_session_str}/"
