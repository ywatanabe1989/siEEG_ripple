...
#     SAMP_RATE_EEG = 200
#     i_time_axis = 1 # fixme
#     ws_sec = cropped_ripples.shape[i_time_axis] / SAMP_RATE_EEG
#     hz = cropped_ripples.sum(axis=i_time_axis) / ws_sec
#     return hz
...
class DataLoaderFiller(object):
    def __init__(
        ...
        tau_ms=250,
        SAMP_RATE_EEG=200,
        apply_sliding_window_data_augmentation=True,
        **kwargs,
    ):
        ...
        self.SAMP_RATE_EEG = SAMP_RATE_EEG
        self.apply_sliding_window_data_augmentation = (
            apply_sliding_window_data_augmentation
        )
        self.i_fold = 0
    ...
    def fill(self, i_fold, reset_fill_counter=True):
        ...
        if self.apply_sliding_window_data_augmentation:
    ...
    def fill_a_dataset(self, subset, rand_init_value, ws_pts, do_shuffle):
        ...
        for i_row, (e, r) in enumerate(zip(EEG, ripples)):
            # e = e.transpose(0, 2, 1)
            E.append(e)
            R.append(r)

        E = np.vstack(E).astype(np.float32).squeeze().transpose(0, 2, 1)
        R = np.vstack(R).astype(np.float32).squeeze()
        S = np.array(subjects).astype(int)


        # # z-norm by subject
        # for ss in S:
        #     indi = np.where(ss == S)[0]
        #     # _E = E[indi].transpose(1,0,2).reshape(6, -1)
        #     # E[indi] = (E[indi] - _E.mean(keepdims=True)) / _E.std(
        #     #     keepdims=True
        #     # )
        #     _R = R[indi].reshape(-1)
        #     R[indi] = (R[indi] - _R.mean(keepdims=True)) / _R.std(
        #         keepdims=True
        #     )

        # # z-norm by electrode
        # E = (E - E.mean(axis=1, keepdims=True)) / E.std(axis=1, keepdims=True)
        # # z-norm by dataset
        # R = 

        I = R.mean(axis=-1) * self.SAMP_RATE_EEG
        # I = (I - I.mean()) / I.std()
...
if __name__ == "__main__":
    ...
    # dlf = DataLoaderFiller(ws_pts=100,
    #                        apply_sliding_window_data_augmentation=True,
    #                        )
    ...
    for i_fold in range(dlf.n_repeat):
        ...
        for epoch in range(n_epochs):
            ...
            for i_batch, batch in enumerate(dlf.dl_val):  # 45 batches * 64 samples
                Eb_val, Rb_val, Ib_val = batch

                # Validate your model

            for i_batch, batch in enumerate(dlf.dl_tra):  # 106
                Eb_tra, Rb_tra, Ib_tra = batch
                print(Rb_tra)
                Is_tra.append(Ib_tra)
    #             print()
    #             print(epoch)
    #             print()
    #             print(Sgb_tra)
